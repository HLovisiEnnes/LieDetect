...
def local_PCA(data, k, n):
    ...
    closest_indices = kdt.query(data, k, return_distance=False)
    for i in range(len(data)): 
        closest_points = [data[j] for j in closest_indices[i]]
        #apply PCA to local data
        pca = PCA(n_components = n)
        pca.fit(closest_points)
...
def basis_Lie(data, Ts_perp = None, dim = None, k = None, n = None, plot = True, save = None):
    ...
    vals = np.real(vals).tolist()
    vals = np.abs(vals)

    dic = {}
    for i,val in enumerate(vals): dic[val]=np.real(vec[i])
    vals.sort()
    vecs = [dic[val] for val in vals] #order the eigenvectors based on eigenvalues
