...
def basis_Lie(data, Ts_perp = None, dim = None, k = None, n = None, plot = True, save = None):
    ...
    Output: (vecs, val, \Sigma) estimated vectors for a basis of the Lie algebra, in order, and the associated eigenvalues, 
    with the operator \Sigma
    '''        
    if dim == None:
        dim = data.shape[1]
